...
import com.gimgongta.chordmoum.service.ChordmoumService.chordList
import com.gimgongta.chordmoum.service.ChordmoumService.chordNoteArrList
...
class Chordmoum : AppCompatActivity() {
    ...
    private fun init() {
        ...
        for (i in buttonArr.indices) {
            buttonArr[i].setOnClickListener {
                binding.chordmoumText.text = "${noteList[baseNote]}${chordList[i]}"
    ...
    private fun playChord(baseNote: Int, index: Int) {
        ...
        println("mylog chordNoteArrList[index] : ${chordNoteArrList[index]}")
        chordNoteArrList[index].forEach {
